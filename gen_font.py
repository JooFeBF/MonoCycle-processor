...
CHAR_H = 16
NUM_CHARS = 256
OUTPUT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "src", "vga_font.hex")
FONT_CANDIDATES = [
...
def generate_font():
    ...
    for code in range(NUM_CHARS):
        if 32 <= code <= 126:
            ...
            rows = [0] * CHAR_H
        table.extend(rows)
    assert len(table) == NUM_CHARS * CHAR_H, f"Expected {NUM_CHARS * CHAR_H}, got {len(table)}"
    return table
...
def main():
    ...
    print("\n--- Sample Glyphs ---")
    for code in [ord(' '), ord('A'), ord('B'), ord('M'), ord('W'),
                 ord('0'), ord('1'), ord('x'), ord(':'), ord('i'),
                 ord('.'), ord('|'), ord('@'), ord('g'), ord('y'),
                 ord('_'), ord('~'), ord('#')]:
